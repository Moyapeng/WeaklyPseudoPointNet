...
class Options:
    def __init__(self, isTrain):
        ...
        self.train['train_epochs'] = 80         # number of training epochs
        self.train['finetune_epochs'] = 0      # number of refinement epochs
        self.train['batch_size'] = 8            # batch size
        # self.train['crf_weight'] = 0.0005       # weight for crf loss
        self.train['crf_weight'] = 0.0000  # weight for crf loss
        ...
        self.train['ema_interval'] = 10  # eps to save ema
        self.train['start_update_epoch'] = 600
        self.train['ema_weight'] = 0.1
        self.train['workers'] = 1               # number of workers to load images
        self.train['gpus'] = [0, ]              # select gpu devices
        self.train['decoder_radius'] =[6,8,10,12]
        # --- resume training --- #
        self.train['start_epoch'] = 0    # start epoch
        # self.train['checkpoint'] = './experiments/cpm_17/checkpoints/checkpoint_60.pth.tar'
    ...
    def parse(self):
        ...
        if self.isTrain:
            ...
            self.train['label_point_dir'] = '{:s}/labels_point'.format(self.train['data_dir'])
            self.train['label_gauss_dir'] = '{:s}/labels_gauss'.format(self.train['data_dir'])
            self.train['label_weight_dir'] = '{:s}/labels_weight'.format(self.train['data_dir'])
            self.train['label_seg_dir'] = '{:s}/labels_seg'.format(self.train['data_dir'])
