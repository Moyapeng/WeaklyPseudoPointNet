...
def compute_metrics(pred, gt, names):
    ...
    if 'dice_pixel' in names:   # pixel-level Dice
        results['dice_pixel'] = dice_coefficient(gt_binary, pred_binary)
...
def dice_coefficient(pred, gt):
    intersection = 2 * np.sum(np.logical_and(pred, gt))
    union = np.sum(pred) + np.sum(gt)

    if union == 0:
        return 1.0  # Handling the case where both masks are empty
