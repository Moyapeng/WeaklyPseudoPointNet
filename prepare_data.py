...
def main():
    ...
    label_seg_dir = './data/{:s}/labels_seg'.format(dataset)
    ...
    create_point_label_from_instance(label_instance_dir, label_point_dir, train_list ,label_seg_dir)
   ...
   # ------ split large images into 250x250 patches
    ...
    split_patches(label_seg_dir, '{:s}/labels_seg'.format(patch_folder), 'label_seg')
    split_patches(label_point_dir, '{:s}/labels_point'.format(patch_folder), 'label_point')
...
def create_point_label_from_instance(data_dir, save_dir, train_list, seg_dir):
    def get_point(img):
        a = np.where(img != 0)
    ...
    for image_name in image_list:
        ...
        imageio.imsave('{:s}/{:s}_seg.png'.format(seg_dir, name), label_seg.astype(np.uint8)*255)
...
def create_cluster_label(data_dir, label_point_dir, label_vor_dir, save_dir, train_list):
    ...
    for img_name in sorted(img_list):
        ...
        print('\t[{:d}/{:d}] Processing image {:s} ...'.format(N_processed, N_total, img_name))
        # ori_image = imageio.imread('{:s}/{:s}'.format(data_dir, img_name))
        ori_image = Image.open('{:s}/{:s}'.format(data_dir, img_name))
        if ori_image.mode == 'RGBA':
            ori_image = ori_image.convert('RGB')
        # ori_image = np.array(Image.open('{:s}/{:s}'.format(data_dir, img_name)))
...
def organize_data_for_training(data_dir, train_data_dir):
    ...
    create_folder('{:s}/labels_seg'.format(train_data_dir))
    create_folder('{:s}/labels_point'.format(train_data_dir))
    ...
    create_folder('{:s}/labels_seg/train'.format(train_data_dir))
    ...
    for img_name in train_list:
        ...
        for file in glob.glob('{:s}/patches/labels_cluster/{:s}*'.format(data_dir, name)):
            ...
            shutil.copyfile(file, dst)
         # label_seg
        for file in glob.glob('{:s}/patches/labels_seg/{:s}*'.format(data_dir, name)):
            file_name = file.split('/')[-1]
            dst = '{:s}/labels_seg/train/{:s}'.format(train_data_dir, file_name)
            shutil.copyfile(file, dst)
        # label_point
        for file in glob.glob('{:s}/patches/labels_point/{:s}*'.format(data_dir, name)):
            file_name = file.split('/')[-1]
            dst = '{:s}/labels_point/train/{:s}'.format(train_data_dir, file_name)
            shutil.copyfile(file, dst)
...
def compute_mean_std(data_dir, train_data_dir):
    ...
    # print(mean_values)
    # print(total_square_sum / num_pixel)
    # print(total_square_sum / num_pixel - mean_values ** 2)
